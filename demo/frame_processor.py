...
class frame_processer:
    ...
    def process_calibration(self, calib_data, mon, device, gaze_network, por_available=False, show=False):

        g_t = None
        data = {'image_a': [], 'gaze_a': [], 'head_a': [], 'R_gaze_a': [], 'R_head_a': []}

        for g_t, frames in zip(calib_data['g_t'], calib_data['frames']):
            for img in frames:
                self.process_frame(data, img, g_t, mon, device, gaze_network, por_available, show)

        return data

    def process_video(self, cap, mon, device, gaze_network, por_available=False, show=False):
        ...
        while cap.isOpened():
            ...
            if not ret:
                break
            self.process_frame(data, img, None, mon, device, gaze_network, por_available, show)

        return data

    def process_frame(self, data, img, g_t, mon, device, gaze_network, por_available=False, show=False):
        # detect face
