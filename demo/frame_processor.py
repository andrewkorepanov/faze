...
class FrameProcessor:
    ...
    def process_calibration(self, calibration_data: pd.DataFrame,
                            monitor: Monitor) -> pd.DataFrame:

        data: pd.Series = calibration_data.apply(lambda row: np.array(
            self._process_calibration_frame(row['frame'], row['gaze'], monitor)
        ),
                                                 axis=1)
    ...
    def _process_calibration_frame(self, image, gaze, monitor):
        print('GAZE: ', gaze)
        # detect face
    ...
    def _process_video_frame(self,
        ...
        if show:
            ...
            output_tracked = self.kalman_filter_gaze[0].update(x_pixel_hat +
                                                               1j *
                                                               y_pixel_hat)
            x_pixel_hat, y_pixel_hat = np.ceil(
                np.real(output_tracked)), np.ceil(np.imag(output_tracked))
            ...
            g_t.append(x_pixel_hat)
            g_t.append(y_pixel_hat)
            ...
            # cv2.imwrite(f'./output/{datetime.datetime.now()}.png', display)
            ...
            # sel.head_pose_estimator.drawPose(img, rvec, tvec, self.cam_calib['mtx'], np.zeros((1, 4)))
